...
export class ShopsComponent {
  ...
  public categories = signal([
    { id: 'food', name: 'Food', icon: 'fas fa-utensils', color: 'bg-yellow-50 text-yellow-600' },
    { id: 'beauty', name: 'Beauty', icon: 'fas fa-magic', color: 'bg-pink-50 text-pink-600' },
    { id: 'electronics', name: 'Electronics', icon: 'fas fa-mobile-alt', color: 'bg-blue-50 text-blue-600' },
    { id: 'fashion', name: 'Fashion', icon: 'fas fa-tshirt', color: 'bg-purple-50 text-purple-600' }
  ]);

  public selectedCategory = signal('food');

  public shops = signal([
    // FOOD SHOPS (Restaurants are also shops in this context)
    {
  ...
  public filteredShops = signal(this.shops());

  constructor() {
    this.filterShops('food');
  }

  filterShops(categoryId: string) {
    this.selectedCategory.set(categoryId);
    this.filteredShops.set(this.shops().filter(shop => shop.category === categoryId));
  }
}
